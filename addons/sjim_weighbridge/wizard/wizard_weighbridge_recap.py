...
class MetroWeighbridgeRecapitulation(models.TransientModel):
    ...
    @api.multi
    def print_beacukai_recap_xlsx(self):
        self.ensure_one()
        date_start = self.start_date or min(self.line_ids.mapped('weighbridge_id.TIMBANG_OUT_DATE'))
        date_stop = self.stop_date or max(self.line_ids.mapped('weighbridge_id.TIMBANG_OUT_DATE'))
        return{
...
class SampitWeighbridgeRecapitulation(models.TransientModel):
    _name = 'wizard.weighbridge.recap.sampit'
    _description = 'Rekapitulasi Timbangan Sampit'
    ...
    purchase_id = fields.Many2one('purchase.order', 'Odoo Purchase Order')
    target_data = fields.Selection([('validated','Valid'),('draft','Belum Valid'),('all','Semua Data')], string='Target Data', required=True, default='all')
    line_ids = fields.One2many('weighbridge.recap.sampit.line', 'wizard_id', 'Data Timbangan')
    start_date = fields.Date('Dari Tanggal')
    stop_date = fields.Date('Sampai Tanggal')
    vehicle_no = fields.Char('No. Kendaraan')
    ...
    def generate_data_timbang(self):
        ...
        WeighbridgeScale = self.env['weighbridge.scale.sampit'].sudo()
        WeighbridgeRecapLine = self.env['weighbridge.recap.sampit.line']
    ...
    def print_internal_recap(self):
        return {
            'type'          : 'ir.actions.report.xml',
            'report_name'   : 'rekap_timbangan_internal_sampit',
            'datas'         : {
                'model'         : 'wizard.weighbridge.recap.sampit',
                'id'            : self.id,
                'ids'           : [self.id],
                'report_type'   : 'xlsx',
                'form'          : {
                    'wizard_metro_id' : 0,
                    'wizard_sampit_id' : self.id,
                },
            },
            'nodestroy': False
        }
    ...
    def print_beacukai_recap(self):
        ...
        return {
            'type'          : 'ir.actions.report.xml',
            'report_name'   : 'rekap_timbangan_beacukai_sampit',
            'datas'         : {
                'model'         : 'wizard.weighbridge.recap.sampit',
                'id'            : self.id,
                'ids'           : [self.id],
...
class SampitWeighbridgeRecapitulationLine(models.TransientModel):
    _name = 'weighbridge.recap.sampit.line'
    _description = 'Data Timbangan'

    wizard_id = fields.Many2one('wizard.weighbridge.recap.sampit', 'Wizard')
    weighbridge_id = fields.Many2one('weighbridge.scale.sampit', 'Tiket', required=True)
    partner_id = fields.Many2one('weighbridge.partner', string='Relasi')
    transporter_id = fields.Many2one('weighbridge.partner', string='Transportir')
    transporter_name = fields.Char(string="Transportir")
    contract_id = fields.Many2one('weighbridge.contract', string='Kontrak')
    ...
    def write(self, update_vals):
        for line in self:
            ...
            line.weighbridge_id.sudo().write(wb_to_update)
        return super(SampitWeighbridgeRecapitulationLine, self).write(update_vals)
        ...
        if res and self.product_id.cost_method == 'average' and average_cost_type=='by_location':
            ...
            if self._context.get('force_valuation_amount'):
                ...
                if self.product_id.cost_method == 'average':
                    ...
                    # valuation_correction_amt = cost if self.location_id.usage == 'supplier' and self.location_dest_id.usage == 'internal' else 
                    if self.location_id.usage == 'supplier' and self.location_dest_id.usage == 'internal':
                        valuation_correction_amt = cost
                    elif self.location_id.usage == 'transit' and self.location_dest_id.usage == 'internal':
                        valuation_correction_amt = self.get_price_unit()
                    else:
                        valuation_correction_amt = self.with_context(location_id=location_to_check.id).get_price_unit_per_location()
                else:
            ...
            if self._context.get('force_valuation_amount'):
                ...
                if self.product_id.cost_method == 'average':
                    ...
                    if self.location_id.usage == 'supplier' and self.location_dest_id.usage == 'internal':
                        ...
                        valuation_amount = self.get_price_unit()
                    else:
                        valuation_amount = self.with_context(location_id=location_to_check.id).get_price_unit_per_location()
                else:
                    valuation_amount = cost if self.product_id.cost_method == 'real' else self.product_id.standard_price
            # the standard_price of the product may be in another decimal precision, or not compatible with the coinage of
            # the company currency... so we need to use round() before creating the accounting entries.

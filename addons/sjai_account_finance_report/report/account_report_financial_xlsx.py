...
from odoo.addons.c10i_account.report.account_report_financial_xlsx import ReportFinancial

class ReportFinancialSJAI(ReportFinancial):
    def _generate_report_balance_periodic(self, wb, sheet, row, style, data, objects, reports, skip_write=False):
        parent_sum_rows = []
        sum_rows = []
        temp = {'balance': 0.0, 'balance_comp': 0.0, 'closing_bal': 0.0}
            ...
            if report.type in ('accounts','account_type'):
                ...
                if skip_write:
                    for account in accounts:
                        debit = result_account.get(account.id,{}).get('debit',0.0)
                        credit = result_account.get(account.id,{}).get('credit',0.0)
                        balance = debit - credit
                        closing_bal = result_account.get(account.id,{}).get('balance',0.0)
                        temp['balance'] += balance*report.sign
                        if objects.enable_filter:
                            debit_comp = result_account_comp.get(account.id,{}).get('debit',0.0)
                            credit_comp = result_account_comp.get(account.id,{}).get('credit',0.0)
                            balance_comp = debit_comp - credit_comp
                            closing_bal_comp = result_account_comp.get(account.id,{}).get('balance',0.0)
                            temp['balance_comp'] += balance_comp*report.sign
                        temp['closing_bal'] += closing_bal*report.sign
                else:
                    ...
                    for account in accounts:
                        ...
                        sheet.write_number(row, 3, balance*report.sign, ll_cell_style_decimal)
                        next_cols = 4
                        if objects.enable_filter:
                            debit_comp = result_account_comp.get(account.id,{}).get('debit',0.0)
                            credit_comp = result_account_comp.get(account.id,{}).get('credit',0.0)
                            balance_comp = debit_comp - credit_comp
                            closing_bal_comp = result_account_comp.get(account.id,{}).get('balance',0.0)
                            sheet.write_number(row, next_cols, balance_comp*report.sign, ll_cell_style_decimal)
                            next_cols += 1
                        sheet.write_number(row, next_cols, closing_bal*report.sign, ll_cell_style_decimal)
                ...
                row, current_total, child_sum_rows = self._generate_report_balance_periodic(wb, sheet, row, style, data, objects, report.account_report_id, skip_write=True)
                temp['balance'] += current_total['balance']*report.sign
                if objects.enable_filter:
                    temp['balance_comp'] += current_total['balance_comp']*report.sign
                temp['closing_bal'] += current_total['closing_bal']*report.sign
                ...
                next_cols = 4
                if objects.enable_filter:
                    sheet.write_number(row, 5, current_total['balance_comp']*report.sign, ll_cell_style_decimal)
                    next_cols += 1
                sheet.write_number(row, next_cols, current_total['closing_bal']*report.sign, ll_cell_style_decimal)
                sum_rows.append(row)
                row+=1
                ...
                if report.show_view_label:
                    ...
                    row+=1
                row, current_total, child_sum_rows = self._generate_report_balance_periodic(wb, sheet, row, style, data, objects, report.children_ids, skip_write=skip_write)
                temp['balance'] += current_total['balance']*report.sign
                if objects.enable_filter:
                    temp['balance_comp'] += current_total['balance_comp']*report.sign
                temp['closing_bal'] += current_total['closing_bal']*report.sign
    ...
    def _generate_report_complete_periodic(self, wb, sheet, row, style, data, objects, reports, skip_write=False):
        parent_sum_rows = []
        sum_rows = []
        temp = {'debit': 0.0, 'credit': 0.0, 'balance': 0.0, 'closing_bal': 0.0}
            ...
            if report.type in ('accounts','account_type'):
                ...
                if skip_write:
                    ...
                    for account in accounts:
                        ...
                        sheet.write_string(row, 2, account.name, ll_cell_style)
                        if report.sign==1:
                            sheet.write_number(row, 3, debit, ll_cell_style_decimal)
                            sheet.write_number(row, 4, credit, ll_cell_style_decimal)
                        else:
                            sheet.write_number(row, 3, credit, ll_cell_style_decimal)
                            sheet.write_number(row, 4, debit, ll_cell_style_decimal)
                        sheet.write_number(row, 5, balance*report.sign, ll_cell_style_decimal)
                        sheet.write_number(row, 6, closing_bal*report.sign, ll_cell_style_decimal)
                        row_end = row
                        row+=1
                    sheet.write_string(row, 1, "", c_hdr_cell_style)
                    sheet.write_string(row, 2, report.name, c_hdr_cell_style)
                ...
                row, current_total, child_sum_rows = self._generate_report_complete_periodic(wb, sheet, row, style, data, objects, report.account_report_id, skip_write=True)
                if report.sign == 1:
                ...
                if row_start == row_end:
                    without_sum = True
                if report.sign == 1:
                    sheet.write_number(row, 3, current_total['debit'], c_hdr_cell_style_decimal)
                    sheet.write_number(row, 4, current_total['credit'], c_hdr_cell_style_decimal)
                else:
                    sheet.write_number(row, 3, current_total['credit'], c_hdr_cell_style_decimal)
                    sheet.write_number(row, 4, current_total['debit'], c_hdr_cell_style_decimal)
                sheet.write_number(row, 5, current_total['balance']*report.sign, c_hdr_cell_style_decimal)
                sheet.write_number(row, 6, current_total['closing_bal']*report.sign, c_hdr_cell_style_decimal)
                sum_rows.append(row)
                row+=1
            elif report.type == 'sum':
                if report.show_view_label:
                    ...
                    row+=1
                row, current_total, child_sum_rows = self._generate_report_complete_periodic(wb, sheet, row, style, data, objects, report.children_ids, skip_write=skip_write)
                if report.sign == 1:
                    temp['debit'] += current_total['debit']
                    temp['credit'] += current_total['credit']
                else:
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        data['form'].update({'date_from': date_from, 'date_to': date_to})
        ...
        for account_report in sorted(objects.account_report_ids, key=lambda x: (x.sequence, x.id)):
            date_string = date_to_string(date_from, date_to, periodic=account_report.report_balance_amount_type=='periodic')
            if objects.enable_filter:
                date_string_cmp = date_to_string(date_from_cmp, date_to_cmp, periodic=account_report.report_balance_amount_type=='periodic')
            ...
            sheet.write_string(2, 1, "", cell_style)
            row = 3
            if objects.debit_credit:
                sheet.write_string(2, 1, "Filter: %s"%date_string, cell_style)
            ...
            if account_report.report_balance_amount_type=='periodic':
                if objects.enable_filter or not objects.debit_credit:
                    column_width = [0, 12, 45, 17, 17, 17]
                    ...
                    next_cols = 4
                    if objects.enable_filter:
                        sheet.write_string(row, next_cols, date_string_cmp, cell_style_center)
                        next_cols += 1
                    sheet.write_string(row, next_cols, "As of %s"%date_string, cell_style_center)
                    next_cols = 5
                    row+=1
                    res = self._generate_report_balance_periodic(workbook, sheet, row, xlsx_style, data, objects, account_report)
                else:
                    column_width = [0, 12, 45, 15, 15, 17, 17]
                    for col_pos in range(1,7):
                        sheet.set_column(col_pos, col_pos, column_width[col_pos])

                    sheet.write_string(row, 1, "Acount Code", cell_style)
                    sheet.write_string(row, 2, "Account Name", cell_style)
                    sheet.write_string(row, 3, "Debit", cell_style)
                    sheet.write_string(row, 4, "Credit", cell_style)
                    sheet.write_string(row, 5, "Balance", cell_style)
                    sheet.write_string(row, 6, "As of %s"%date_string, cell_style)
                    row+=1
                    res = self._generate_report_complete_periodic(workbook, sheet, row, xlsx_style, data, objects, account_report)
                    next_cols = 7
            else:
                if objects.enable_filter or not objects.debit_credit:
                    column_width = [0, 12, 45, 17, 17, 17]
...
ReportFinancialSJAI('report.report_sjai_financial_xlsx', 'accounting.report')
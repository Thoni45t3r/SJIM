...
class AdjustmentsInventory(models.Model):
    ...
    def _get_inventory_lines_values(self):
        ...
        if not self.use_adjustment_date: 
            ...
            temp_args = args+args
            self.env.cr.execute("""SELECT product_id, sum(product_qty) as product_qty, location_id, company_id, lot_id as prod_lot_id, product_packaging as package_id from 
                    (
                    SELECT product_id, sum(product_uom_qty) as product_qty, location_dest_id as location_id, company_id, restrict_lot_id as lot_id, product_packaging 
                    FROM stock_move
                    WHERE %s
                    GROUP BY product_id, location_dest_id, company_id, restrict_lot_id, product_packaging
                    UNION ALL
                    SELECT product_id, sum(-product_uom_qty) as product_qty, location_id as location_id, company_id,  restrict_lot_id as lot_id, product_packaging
                    FROM stock_move
                    WHERE %s
                    GROUP BY product_id, location_id, company_id, restrict_lot_id, product_packaging
                    ) 
                    as stock_move
                    GROUP BY product_id, location_id, company_id, lot_id, product_packaging"""% (domain_adjustments, domain), temp_args)
    ...
    def _compute_theoretical_qty(self):
        if self.inventory_id.use_adjustment_date:
            ...
            self.env.cr.execute("""SELECT product_id, sum(product_qty) as product_qty, location_id, company_id, lot_id as prod_lot_id, product_packaging as package_id from 
                    ...
                    as stock_move
                    GROUP BY product_id, location_id, company_id, lot_id, product_packaging"""%(domain_dest, domain_source), args+args)

...
class ReportTransportDataSale(ReportXlsx):

	def get_data(self, objects):
		domain = [('date', '>=', objects.date_start), ('date', '<=', objects.date_end), ('contract_id.trans_type', '=', 'sale')]
	...
	def generate_xlsx_report(self, workbook, data, objects):
		...
		sheet = workbook.add_worksheet('Transport Report Penjualan')
		...
		sheet.write(1, 0, "REKAPITULASI PENIMBANGAN PENJUALAN", format1)
		...
		sheet.merge_range(row,14,row,16, "TIMBANGAN ASAL/MUAT", format4)
		sheet.write(row+1,14, "BRUTO", format4)
		sheet.write(row+1,15, "TARRA", format4)
		sheet.write(row+1,16, "NETTO", format4)
		sheet.merge_range(row,17,row,19, "TIMBANGAN AKHIR/BONGKAR", format4)
		...
		for data in data_ids.sorted(lambda x: x.date):
			...
			incoterm = data.contract_id.related_sale_id.incoterm.name if data.contract_id.related_sale_id else '-'
...
ReportTransportDataSale('report.report_transport_data_sale_xlsx',
				 'stock.transport.report.sale.wizard')
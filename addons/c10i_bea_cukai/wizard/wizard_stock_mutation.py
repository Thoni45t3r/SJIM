...
class BeacukaiStockMutation(models.TransientModel):
    ...
    def action_generate_value(self):
        ...
        for product in products:
            ...
            default_domain_incoming_partner = [('location_id.usage','in',['customer','supplier']),('location_dest_id.usage','=','internal'),('location_dest_id.kawasan_berikat','=',True),('picking_id','!=',False),('picking_id.bea_cukai_ids','!=',False)]
            default_domain_incoming_inventory = [('location_id.usage','=',['inventory','procurement','production']),('location_dest_id.usage','=','internal'),('location_dest_id.kawasan_berikat','=',True)]
            
            default_domain_outgoing_partner = [('location_id.usage','=','internal'),('location_dest_id.usage','in',['customer','supplier']),('location_id.kawasan_berikat','=',True),('picking_id','!=',False),('picking_id.bea_cukai_ids','!=',False)]
            default_domain_outgoing_inventory = [('location_id.usage','=','internal'),('location_dest_id.usage','in',['inventory','procurement','production']),('location_id.kawasan_berikat','=',True)]

            # Take Opening Entries
            # 1. From Stock Move
            op_domain_move_in_partner = default_domain[:] + default_domain_incoming_partner[:]
            op_domain_move_in_partner.extend([('date','<',date_start)])
            op_domain_move_in_inv = default_domain[:] + default_domain_incoming_inventory[:]
            op_domain_move_in_inv.extend([('date','<',date_start)])
            move_in_partner = StockMove.search(op_domain_move_in_partner)
            move_in_inv = StockMove.search(op_domain_move_in_inv)
            
            op_domain_move_out_partner = default_domain[:] + default_domain_outgoing_partner[:]
            op_domain_move_out_partner.extend([('date','<',date_start)])
            op_domain_move_out_inv = default_domain[:] + default_domain_outgoing_inventory[:]
            op_domain_move_out_inv.extend([('date','<',date_start)])
            move_out_partner = StockMove.search(op_domain_move_out_partner)
            move_out_inv = StockMove.search(op_domain_move_out_inv)
            
            opening_qty = (sum(move_in_partner.mapped('product_qty'))+sum(move_in_inv.mapped('product_qty'))) \
                    - (sum(move_out_partner.mapped('product_qty'))+sum(move_out_inv.mapped('product_qty')))
            # 2. From Other Stock
            if self.product_type.code=='asset':
            ...
            if self.product_type.code=='asset':
                ...
                incoming_qty = sum(move_incoming.mapped('product_qty')) + sum(other_stock_in.mapped('product_qty'))
            else:
                incoming_qty = sum(move_incoming.mapped('product_qty')) + sum(move_incoming2.mapped('product_qty'))
            ...
            if self.product_type.code == 'finish_good':
                ...
                domain_move_out.extend([('location_id.usage','=','internal'),('location_dest_id.usage','in',['customer','supplier']),('location_id.kawasan_berikat','=',True)])
                domain_move_out.extend([('picking_id.bea_cukai_ids','!=',False)])
            ...
            if self.product_type.code == 'asset':
                ...
                outgoing_qty = sum(move_outgoing.mapped('product_qty')) + sum(other_stock_out.mapped('product_qty'))
            else:
                outgoing_qty = sum(move_outgoing.mapped('product_qty')) + sum(move_outgoing2.mapped('product_qty'))
            ...
            line_vals.update({'adjustment_qty': sum(move_adj_in.mapped('product_qty')) - sum(move_adj_out.mapped('product_qty'))})
    ...
    def live_print_report(self):
        report_name = 'beacukai_laporan_mutasi_live_print'
        if self.report_type == 'xlsx':
            report_name = 'beacukai_laporan_mutasi_live_print'
        return {
            'type'          : 'ir.actions.report.xml',

...
class ProductValuation(models.Model):
    ...
    def compute_product_value(self):
        ...
        avg_sale_price = delivered_qty = qty_sale = value_sale = 0.0
        ...
        if intransit_moves:
            for x in intransit_moves:
                return_moves = MoveLine.search([('product_id', '=', product.id), ('location_dest_id.usage', '=', 'internal'),
                                                   ('location_id.usage', '=', 'customer'), ('state', '=', 'done'),
                                                   ('origin_returned_move_id', '=', x.id)])
                if return_moves:
                    continue
                ...
                intransit_move_ids.append(x.id)
        else:
            stock_intransit_qty = 0.0
        ...
        # qty_closing = qty_available - qty_sale - stock_intransit_qty + adjustment_qty
        ...
        self.write({
            ...
            'delivery_intransit_move_ids': intransit_move_ids and [(6,0,intransit_move_ids)] or [],
    ...
    def cancel(self):
        for valuation in self:
            ...
            valuation.write({
                'state':'draft',
                'delivery_intransit_move_ids': [],
                'sale_account_move_ids': [], 
                'opening_account_move_ids': [],
                'purchase_account_move_ids': [],
                'cost_account_move_ids': []})
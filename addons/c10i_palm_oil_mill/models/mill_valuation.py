...
class MillValuation(models.Model):
    ...
    def compute_consume_product_value(self):
        ...
        domain1.extend([('account_id','=',opening_account.id),
            ('product_id','=',product.id)])
        move_opening_bal = AccountMoveLine.search(domain1, limit=2) # saldo awal asli dan adjustment (jika ada)
        opening_qty = opening_value = 0.0
        if move_opening_bal:
            for opmove in move_opening_bal:
                op_value = opmove.debit - opmove.credit
                opening_value += op_value
                osign = 1 if op_value > 0 else -1
                if product.uom_id.id!=opmove.product_uom_id.id:
                    opening_qty += osign * opmove.product_uom_id._compute_quantity(opmove.quantity, product.uom_id)
                else:
                    opening_qty += osign * opmove.quantity
        ...
        #6. Compute Adjustment Qty
        adjustment_qty_in = adjustment_qty_out = adjustment_qty = 0.0
        adjustment_in_lines = MoveLine.search([('product_id','=',product.id),
            ('location_id.usage','=','inventory'),('location_dest_id.usage','=','internal'),
            ('date','>=',date_start),('date','<=',date_stop),
            ('state','=','done'),
            ])
        if adjustment_in_lines:
            for x in adjustment_in_lines:
                if product.uom_id.id!=x.product_uom.id:
                    adjustment_qty_in += x.product_uom._compute_quantity(x.product_uom_qty, product.uom_id)
                else:
                    adjustment_qty_in+= x.product_uom_qty
        adjustment_out_lines = MoveLine.search([('product_id','=',product.id),
            ('location_id.usage','=','internal'),('location_dest_id.usage','=','inventory'),
            ('date','>=',date_start),('date','<=',date_stop),
            ('state','=','done'),
            ])
        if adjustment_out_lines:
            for x in adjustment_out_lines:
                if product.uom_id.id!=x.product_uom.id:
                    adjustment_qty_out += x.product_uom._compute_quantity(x.product_uom_qty, product.uom_id)
                else:
                    adjustment_qty_out+= x.product_uom_qty
        adjustment_qty = adjustment_qty_in - adjustment_qty_out
        
        #7. Compute Closing Balance
        qty_closing = qty_available - consume_qty + adjustment_qty
        value_closing = qty_closing * avg_price
        consume_line_dict.update({'adjustment_qty': adjustment_qty, 
            'closing_qty': qty_closing, 'closing_value': value_closing})
    ...
    def create_move_produce_product_value(self, amount_cogm):
        ...
        for produce_product in self.bom_id.mapped('bom_line_ids.product_id'):
            ...
            avg_sale_price = delivered_qty = sales_qty = sales_value = qty_produce = 0.0
            ...
            if move_sales:
                ...
                invoices = move_sales.mapped('invoice_id')
                for inv in invoices:
                    invmoves = self.env['stock.move']
                    for invline in inv.invoice_line_ids:
                        for invmove in invline.move_line_ids.filtered(lambda x: x.state=='done'):
                            invmoves |= invmove
                    delivered_qty += sum(invmoves.mapped('product_qty')) if invmoves else 0.0
            else:
        ...
        for produce_product in self.bom_id.mapped('bom_line_ids.product_id'):
            ...
            domain1.extend([('account_id', '=', opening_account.id),
                            ('product_id', '=', product.id)])
            move_opening_bal = AccountMoveLine.search(domain1, limit=2)
            opening_qty = opening_value = 0.0
            if move_opening_bal:
                for opmove in move_opening_bal:
                    op_value = opmove.debit - opmove.credit
            ...
            delivered_qty = produce_products[product]['delivered_qty']
            # cogs_value = sale_qty * avg_price
            cogs_value = delivered_qty * avg_price
            ...
            # qty_closing = qty_available - sale_qty - stock_intransit_qty + adjustment_qty
            qty_closing = qty_available - delivered_qty - stock_intransit_qty + adjustment_qty
            value_closing = qty_closing * avg_price
...
class MillValuationConsumeLine(models.Model):
    ...
    adjustment_qty = fields.Float('Adjustment Qty', digits=dp.get_precision('Product Unit of Measure'))
...
class MillValuationConsumeLine(models.Model):
    ...
    delivered_qty = fields.Float('Delivered Qty (Invoiced)', digits=dp.get_precision('Product Unit of Measure'))
